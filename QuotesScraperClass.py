...
class QuotesScraper:
    ...
    def __scrape_page(self, url):
        ...
        return [{
            'text': quote.find_element(By.CLASS_NAME, 'text').text.replace('\u201C','').replace('\u201D',''),
    ...
    def __wait_for_load(self):
        WebDriverWait(self.__driver, self.__timeout).until(expected_conditions.presence_of_element_located((By.CLASS_NAME, 'quote')))
        return
